...
# Manual install
# CC="gcc -fPIC" --enable-shared are required for ubuntu AMD64
...
# Matplotlib 1.4.0rc2
wget https://github.com/matplotlib/matplotlib/archive/v1.4.0rc2.tar.gz
tar -xzvf v1.4.0rc2.tar.gz matplotlib-1.4.0rc2/
cd matplotlib-1.4.0rc2/
sudo python3.4 setup.py install
cd ../
sudo rm matplotlib-1.4.0rc2/ -r
rm v1.4.0rc2.tar.gz
...
cd zlib-1.2.8/
./configure CC="gcc -fPIC" --enable-shared
make
sudo make install
cd ../
...
cd szip-2.1/
./configure --prefix=/usr/local CC="gcc -fPIC" --enable-shared
make
make check
sudo make install
make check-install
